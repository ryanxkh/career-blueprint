...
- Proceed conversationally with 2-4 questions at a time
...
- **GTM bias test completed** with 7 personas (Solutions Engineer, Enterprise AE, SDR, VP RevOps, Sales Manager, Diesel Mechanic, Staff SWE). Average score 9.29 across all personas with a GTM vs. non-GTM delta of 0.05 — no meaningful bias detected. Methodology confirmed as genuinely universal; surface-level vocabulary skews slightly white-collar but does not affect coaching quality.

**Changes in v3.4 (from v3.3):**
- **Normalized multiple entry formats** in CONTEXT and GETTING STARTED — resume, portfolio, client list, LinkedIn, or verbal walkthrough are all valid starting points. Eliminated two-tier "resume preferred / no resume okay" framing
- **Tightened the opening message** — combined materials request and discovery question into a single "two things" structure. Removed the separate "if you don't have a resume" reassurance paragraph that created a lesser-path feeling
- **Added "thin resume" path to Phase 1** — explicit guidance for the most common real-world scenario: materials that provide a skeleton but lack accomplishment detail. Coach acknowledges what's there, asks targeted questions to fill depth, avoids framing the gap as a deficiency
- **CONTEXT section rewritten** as coach-facing guidance ("Materials") rather than a user-facing quality gate ("Before Starting")

**Changes in v3.3 (from v3.2):**
- Added **strong resume recommendation** to CONTEXT and GETTING STARTED — positioned as "works best with a resume" rather than a hard requirement, with quality guidance (STAR method, quantifiable impact)
- **Removed redundant opening questions** — no longer asks for current role and tenure in the opening message (resume covers this, and the no-materials fallback in Phase 1 handles it when needed)
- **Streamlined the opening** to lead with the single most important discovery question: "What prompted this?"
- Added **coach-facing note** in Phase 1's no-materials path about spending extra time to compensate for missing resume context

**Changes in v3.2 (from v3.1):**
- Renamed output from "Career Development Tracker" to **"Career Blueprint"** — reframed the entire prompt from "fill in your existing tracker" to "build a document together through conversation"
- Updated CONTEXT, TASK, GETTING STARTED, and OUTPUT FORMAT sections to set user expectations upfront: they'll walk away with a polished Career Blueprint document
- Removed assumption that user already has a template or tracker

**Changes in v3.1 (from v3.0):**
- Added instruction to **explicitly teach the skill/experience test** to users, not just apply it internally
- Expanded **"should" language detection** to include field-specific equivalents ("the expected path," "the standard progression," "that's just how this industry works")
- Added **career-stage awareness cues** to Goal Setting Framework (early, mid, and late career adjustments)
- Broadened **accountability options** beyond individual partners (peer groups, professional communities, masterminds, structured programs, self-review cadence)
- Added **freelance/portfolio career addendum** to Goal Setting Framework (revenue milestones, client quality, pricing power, business structure as progression markers)

**Changes in v3.0 (from v2):**
- Added **Metacognitive Transparency** principle (coach bias-checking) from Claude v2
- Added **"Should" Language Detection** as explicit critical behavior from v1
- Added **Constraint Reframing** guidance from Gemini v2
- Added **Efficiency Checkpoint** around exchange 15-20 from Claude v2
- Added **Evidence Log** intervention for impostor syndrome from Claude v2
- Added **Immediate Actions** requirement to ensure momentum from experiment findings
- Added **Accountability Integration** from experiment findings
- Added periodic synthesis guidance (every 8-10 exchanges) from Gemini v2
- Consolidated questioning themes from 5 to 3 areas for better navigation
- Reduced clinical process narration per Gemini v2 feedback
- Enhanced output template with Reflection & Alignment section
- Added example patterns for key coaching moments
- De-biased all examples from sales/tech-specific to cross-industry
